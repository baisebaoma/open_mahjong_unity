"""Hongque heuristic v3 bot (user_id=3): score-aware tile play with folding.

Defense layer on top of the v2 heuristic:

- **Genbutsu safety**: the Hongque deck holds one copy of each tile, so a tile
  an opponent has already discarded is safe against that opponent.
- **Building-direction danger**: opponents' open melds reveal the colour family
  and number range they are building.  A discard near that region is risky;
  a family an opponent sheds heavily is cheap.
- **Threat-gated folding**: only when the round is late and opponents' rivers
  are long does v3 leave pure offense.  A close hand keeps attacking and merely
  prefers the safer tile on ties; a far hand folds toward the safest discard
  within a small replacement-distance budget.

Tenpai-wait selection, claims, supplements and kong stay identical to v2.
