"""Hongque heuristic v6 bot (user_id=3): v3 + tenpai tie-break on safety.

Experimental change over ``heuristic_bot_v3`` (see
HONGQUE_HEURISTIC_RESEARCH.md §10):

- **E3: tenpai tie-break on safety** — when several discards keep tenpai with
  identical live-wait count and points, v2/v3 always shed the drawn tile.
  With 76% of Hongque wins coming from ron, a dangerous draw should instead
  be replaced by an equally good safe discard.  E3 picks the least dangerous
  tile among (points, ukeire) ties, falling back to the drawn tile whenever
  it is not strictly more dangerous than the safe alternative.

Unlike v5 (D1+D3, which over-folds and loses to v3), v6 makes **no threat
gating or claim changes** — it only reorders which tile is discarded when the
hand is already tenpai and the discard choice is otherwise a wash.  Everything
else (folding, claims, supplements, kong) is identical to v3.  This module is
deliberately import/mutation-free.
"""
...
def _pick_tenpai_discard(ties: Sequence[str], drawn_tile: Optional[str],
                         opponents: Sequence[OpponentView]) -> str:
    """E3: among equal (points, ukeire) tenpai discards prefer the safe one.

    v2/v3 always shed the drawn tile.  E3 only deviates when the drawn tile
    is strictly more dangerous than an equally good alternative — otherwise it
    keeps the original behavior.
    """
    if drawn_tile in ties:
        drawn_danger = _tile_danger(drawn_tile, opponents)
        safest = min(
            (t for t in ties if t != drawn_tile),
            key=lambda t: (_tile_danger(t, opponents), t),
            default=None,
        )
        if safest is not None and _tile_danger(safest, opponents) < drawn_danger:
            return safest
        return drawn_tile
    return min(ties, key=lambda t: (_tile_danger(t, opponents), t))
...
def choose_discard(
    ...
    *,
    ...
    if ready:
        ...
        if len(ties) > 1:
            best_tile = _pick_tenpai_discard(ties, drawn_tile, opponents)
        elif drawn_tile in ties:
...
def choose_claim_plan(
    ...
    *,
    ...
    Identical to v3: opponents/wall_count are accepted for seat symmetry but
    unused — v6 keeps v3's claim behaviour (E3 only touches tenpai discards).
    """
