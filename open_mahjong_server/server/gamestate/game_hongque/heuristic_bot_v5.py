"""Hongque heuristic v5 bot (user_id=3): v3 + D1 + D3 (experimental, loses).

Two experimental changes over ``heuristic_bot_v3`` (see
HONGQUE_HEURISTIC_RESEARCH.md §10; this combination benchmarks **worse**
than v3 — 2.748 vs ~2.42, kept only as an experiment record):

- **D1: meld-weighted threat** — ``_threat_level`` also counts opponents'
  open melds.  An opponent with several open groups is closer to tenpai, so
  their threat should rise even when their river is still short.  The meld
  factor multiplies the existing late-game × river-length threat.
- **D3: claim suppression under threat** — when threat is high and the
  discard a claim would force stays dangerous (danger > cap), pass instead.

Everything else (tenpai-wait selection, supplements, kong, tile danger)
is identical to v3.  This module is deliberately import/mutation-free.
"""
...
# D1: each open meld group adds 50% threat; cap the multiplier.
_MELD_THREAT_WEIGHT = 0.5
_MELD_THREAT_CAP = 1.5
...
def _threat_level(opponents: Sequence[OpponentView], wall_count: int) -> float:
    ...
    base = late * (avg_discards / 8.0)
    avg_melds = sum(max(0, len(o.meld_tiles) // 3) for o in opponents) / len(opponents)
    meld_factor = 1.0 + min(_MELD_THREAT_CAP, avg_melds * _MELD_THREAT_WEIGHT)
    return min(1.0, base * meld_factor)
...
# after the v5 folding logic has already picked its safest option.
